...
local creationDestructionActions = {
    {
        ...
        func = function()
            ...
            if block.find(block, "minecraft:arrow") then
              host:sendChatCommand(("summon %s ~ ~1 ~ {Fuse:40,BlockState:{Name:\"%s\"},Motion:[%f, %f, %f]}"):format(block, block, motion.x, motion.y, motion.z))
            elseif block.find(block, "_bucket") then
              local liquid = string.gsub(block, "_bucket", "")
              host:sendChatCommand(("summon %s ~ ~1 ~ {Fuse:40,BlockState:{Name:\"%s\"},Motion:[%f, %f, %f]}"):format("minecraft:falling_block", liquid, motion.x, motion.y, motion.z))
            elseif block.find(block, "_spawn_egg") then
              local creature = string.gsub(block, "_spawn_egg", "")
              host:sendChatCommand(("summon %s ~ ~1 ~ {Fuse:40,BlockState:{Name:\"%s\"},Motion:[%f, %f, %f]}"):format(creature, creature, motion.x, motion.y, motion.z)) 
            elseif block.find(block, "minecraft:tnt") then
              host:sendChatCommand(("summon %s ~ ~1 ~ {Fuse:40,BlockState:{Name:\"%s\"},Motion:[%f, %f, %f]}"):format(block, block, motion.x, motion.y, motion.z))
            elseif block == "minecraft:fire_charge" then
              host:sendChatCommand(("summon %s ~ ~1 ~ {Fuse:40,BlockState:{Name:\"%s\"},Motion:[%f, %f, %f]}"):format("minecraft:fireball", block, motion.x, motion.y, motion.z))
            else
              host:sendChatCommand(("summon %s ~ ~1 ~ {Fuse:40,BlockState:{Name:\"%s\"},Motion:[%f, %f, %f]}"):format("minecraft:falling_block", block, motion.x, motion.y, motion.z))
            end
        end
    },
    {
...
for i, v in pairs(creationDestructionActions) do
    iter = iter + 1
    pages[1].page:newAction(i+1):title(v.title):setItem(v.item):setOnLeftClick(v.func):color(0.2,0.2,0.2)
end
while iter % 8 ~= 0 do
...
for i, v in pairs(difficultyActions) do
    iter = iter + 1
    difficultyPage:newAction(i+1):title(v.difficulty:gsub("^.", string.upper)):item(v.item):setOnLeftClick(function ()
...
for i, v in pairs(gamemodeActions) do
    iter = iter + 1
    gamemodePage:newAction(i+1):title(v.gamemode:gsub("^.", string.upper)):item(v.item):setOnLeftClick(function ()
        host:sendChatCommand("difficulty " .. v.difficulty)
    end):color(0.2,0.2,0.2)
end
while iter % 8 ~= 0 do
...
for i, v in ipairs(cheats) do
    iter = iter + 1
    pages[2].page:newAction(i+1):title(v.title):item(v.item):setOnLeftClick(v.func):color(0.2,0.2,0.2)
end
while iter % 8 ~= 0 do
