...
function events.render()
    ...
    if not player:getVehicle() or bypassHealth then
        ...
        UI:newText("Health"):text(toJson({
            ...
            {text = ": ", color = "gray"},
            { text = tostring(math.round(player:getHealth() + player:getAbsorptionAmount())), color = (function()
    ...
    UI:newText("Armor"):text(toJson({
        ...
        {text = ": ", color = "gray"},
        { text = tostring(math.round(player:getArmor())), color = (function()
            local armor = player:getArmor()
            local armorPercent = (armor / 20) * 100
-- if player:getExhaustion() > 1 then log(player:getExhaustion()) end
            return '#' .. vectors.rgbToHex(healthGradient[math.clamp(math.round(armorPercent), 1, 100)] / 255)
        end)()},
        { text = " / ", color = "gray" },
        { text = "20", color = "white" }
    })):pos(vec((size.x / 2) - 50, size.y - 52, 0) * -1):alignment("CENTER"):setBackground(true):setBackgroundColor(0, 0, 0, 0.5)
    ...
    })):pos(vec((size.x / 2) + 50, size.y - 64, 0) * -1):alignment("CENTER"):setBackground(true):setBackgroundColor(0, 0, 0, 0.5)
    ...
    UI:newText("Hunger"):text(toJson({
        ...
        {text = ": ", color = "gray"},
        { text = tostring(math.round(player:getFood())), color = (function()
            ...
            return '#' .. vectors.rgbToHex(healthGradient[math.clamp(math.round(hungerPercent), 1, 100)] / 255)
        end)() },
        { text = " / ", color = "gray" },
    ...
    UI:newText("Saturation"):text(toJson({
        ...
        {text = ": ", color = "gray"},
        { text = tostring(math.round(player:getSaturation())), color = (function()
            ...
            return '#' .. vectors.rgbToHex(healthGradient[math.clamp(math.round(saturationPercent), 1, 100)] / 255)
        end)() },
        { text = " / ", color = "gray" },
    ...
    UI:newText("Air"):text(toJson({
        { text = "Air", color = "aqua" },
        {text = ": ", color = "gray"},
        { text = tostring(math.round(math.clamp(player:getNbt().Air, 0, math.huge))), color = (function()
            ...
            return '#' .. vectors.rgbToHex(healthGradient[math.clamp(math.round(airPercent), 1, 100)] / 255)
        end)() },
        { text = " / ", color = "gray" },
        { text = tostring(math.round(player:getMaxAir())), color = "aqua" }
    })):pos(vec((size.x / 2), size.y - 76, 0) * -1):alignment("CENTER"):setBackground(true):setBackgroundColor(0, 0, 0, 0.5):setVisible(player:isInWater())
end
